...
public class ElasticsearchEmitter implements IEmitter<ElasticsearchObject> {
    ...
    public ElasticsearchEmitter(KinesisConnectorConfiguration configuration) {
        ...
        elasticsearchClient.addTransportAddress(new TransportAddress(new InetSocketAddress(elasticsearchEndpoint, elasticsearchPort)));
    }
    ...
    public List<ElasticsearchObject> emit(UnmodifiableBuffer<ElasticsearchObject> buffer) throws IOException {
        ...
        for (ElasticsearchObject record : records) {
            ...
            // apply fix by https://stackoverflow.com/questions/39187097/elastic-search-number-of-object-passed-must-be-even
            JSONObject dataAsJson = new JSONObject(record.getSource());
            HashMap<String, Object> dataAsMap = new HashMap<String, Object>(dataAsJson.toMap());
            indexRequestBuilder.setSource(dataAsMap, XContentType.JSON);
